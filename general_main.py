...
def main(args):
    ...
    result = multiple_run(args, store=args.store, save_path=args.save_path)
    if args.online:
        print("{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}".format(args.num_runs,args.retrieve,args.learning_rate,args.epoch,args.batch,args.data,args.mem_size,args.eps_mem_batch,args.online,round(result[0][0],5),round(result[0][1],5),round(result[1][0],5),round(result[1][1],5),round(result[2][0],5),round(result[2][1],5),round(result[3][0],5),round(result[3][1],5),round(result[4][0],5),round(result[4][1],5)))
    else:
        print("{},{},{},{},{},{},{},{},{}".format(args.num_runs,args.retrieve,args.learning_rate,args.epoch,args.batch,args.data,args.mem_size,args.online,round(result,5)))
...
if __name__ == "__main__":
    ...
    ########################General#########################
    #训练的轮数，每次训练都会构建新的十个任务进行学习，模型也是从零开始
    parser.add_argument('--num_runs', dest='num_runs', default=1, type=int,
                        help='Number of runs (default: %(default)s)')
    #随机训练，在进行多次试验取平均值和计算误差和方差大小时使用
    parser.add_argument('--random_train', dest='random_train', default=False, type=bool,
                        help='是否使用随机种子进行训练')
    #随机种子
    ...
    parser.add_argument('--test_batch', dest='test_batch', default=256,
                        type=int,
    ...
    a = time.perf_counter()
    main(args)
    print("本次程序运行用时为：",time.perf_counter()-a)
# 配置
# 用时
# 单卡 --data cifar100 --cl_type nc --agent ER --update ASER --retrieve ASER --mem_size 5000 --aser_type asvm --n_smp_cls 1.5 --k 3
# 用时 800.2049002379645
